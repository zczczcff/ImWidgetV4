...
目的：让编辑器表达“一个真实应用工程”，而不是只表达“一个打开中的 UI 文档”。
...
目的：从编辑器内发起 configure/build，并把结果组织成可消费信息。
...
1. 项目 manifest 与 `EditorProject`
2. 新建项目向导
3. 目录骨架生成
4. CMake 生成
5. 桌面平台 configure / build 跑通
6. Android 工具链检测与 Android 工程生成
7. Android configure / build 接入

## 风险点

- 自动生成文件与用户手写文件边界不清，后续再生成容易破坏用户工程
- 不同平台入口和资源组织方式差异大，若前期模型不稳定，后面扩平台会返工
- 工具链检测若只写死 Windows 逻辑，会再次形成平台耦合
- 构建输出如果不做结构化解析，后续错误导航和一键修复难扩展

## 验收标准

当满足以下条件时，可认为这一阶段达标：

- 编辑器可创建一个新的跨平台 GUI 项目
- 新项目可自动生成目录、UI 文档和 CMake 工程
- 用户可在编辑器中继续编辑 UI
- 编辑器可一键执行桌面 configure/build
- Android 平台至少具备环境检测与配置入口
- 文档、模板、构建配置都能稳定重复生成
