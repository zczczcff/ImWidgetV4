...
class FCanvasPanelDemoHostDelegate : public IApplicationHostDelegate {
public:
    FApplicationHostConfig GetHostConfig() const override
    {
        FApplicationHostConfig config;
        config.Title = "CanvasPanel Demo - ImWidgetV4";
        config.InitialWidth = 960;
        config.InitialHeight = 640;
#if defined(_WIN32)
        config.IniSettingsPath = Samples::GetDefaultSampleImGuiIniPath(L"CanvasPanelDemo.ini");
#endif
        return config;
    }
    ...
    void ConfigureApplication(ImApplication& application) override
    {
        ...
        overlapBack->OnClicked.AddLambda([status](ImButton&) {
            status->SetText("Back-layer button clicked.");
        });

        overlapFront->OnClicked.AddLambda([status](ImButton&) {
            status->SetText("Front-layer button clicked.");
        });

        autoCheckBox->OnCheckStateChanged.AddLambda([status](ImCheckBox&, bool checked) {
            status->SetText(checked ? "Auto-sized CheckBox enabled." : "Auto-sized CheckBox disabled.");
        });

        autoSlider->OnValueChanged.AddLambda([status](ImSlider&, float value) {
            status->SetText("Auto-sized Slider value: " + std::to_string(static_cast<int>(value)));
        });

        application.SetRootWidget(canvas);
    }
};
...
{
    return std::make_shared<FCanvasPanelDemoHostDelegate>();
}
