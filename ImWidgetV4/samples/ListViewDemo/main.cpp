...
class FListViewDemoHostDelegate : public IApplicationHostDelegate {
public:
    FApplicationHostConfig GetHostConfig() const override
    {
        FApplicationHostConfig config;
        config.Title = "ListView Demo - ImWidgetV4";
        config.InitialWidth = 1180;
        config.InitialHeight = 760;
#if defined(_WIN32)
        config.IniSettingsPath = Samples::GetDefaultSampleImGuiIniPath(L"ListViewDemo.ini");
#endif
        return config;
    }
...
{
    return std::make_shared<FListViewDemoHostDelegate>();
}
