...
TEST(EditorSelectionTest, WorkspaceControllerCreateAppProjectAtCreatesManifestAndStartupDocument)
{
    ...
    const std::filesystem::path userProjectCMakePath = projectRoot / "cmake" / "UserProject.cmake";
    ...
    EXPECT_TRUE(std::filesystem::exists(generatedCMakePath));
    EXPECT_TRUE(std::filesystem::exists(userProjectCMakePath));
    ...
    EXPECT_TRUE(std::filesystem::exists(titleBarGeneratedSourcePath));
    {
        ...
        EXPECT_NE(text.find("include(cmake/ImWidgetV4GeneratedProject.cmake)"), std::string::npos);
        EXPECT_NE(text.find("include(cmake/UserProject.cmake)"), std::string::npos);
        EXPECT_EQ(text.find("add_subdirectory(\"${IMWIDGETV4_ROOT}\""), std::string::npos);
        EXPECT_EQ(text.find("generated/AppProjectConfig.cpp"), std::string::npos);
    }
    {
        ...
        buffer << stream.rdbuf();
        const std::string text = buffer.str();
        EXPECT_NE(text.find("Auto-generated by ImWidgetV4Editor"), std::string::npos);
    ...
    {
        std::ifstream stream(userProjectCMakePath, std::ios::binary);
        std::stringstream buffer;
        buffer << stream.rdbuf();
        const std::string text = buffer.str();
        EXPECT_NE(text.find("User-owned CMake extension file."), std::string::npos);
    }
    {
    ...
    }
    {
        ...
        EXPECT_NE(text.find("ConfigureApplication(ImWidgetV4::ImApplication& application)"), std::string::npos);
        ...
        EXPECT_EQ(text.find("FGeneratedAppHostDelegate"), std::string::npos);
        EXPECT_EQ(text.find("CreateApplicationHostDelegate"), std::string::npos);
        EXPECT_EQ(text.find("bool InitializeApplication"), std::string::npos);
        EXPECT_EQ(text.find("void Tick"), std::string::npos);
    }
...
TEST(EditorSelectionTest, WorkspaceControllerRegenerateProjectCodeRewritesGeneratedFiles)
{
    ...
    {
        std::ofstream stream(rootCMakeListsPath, std::ios::binary | std::ios::trunc);
        stream << "# user root cmake\ninclude(cmake/ImWidgetV4GeneratedProject.cmake)\n";
    }
    {
        std::ofstream stream(userProjectCMakePath, std::ios::binary | std::ios::trunc);
        stream << "# user custom cmake\nset(USER_CMAKE_SENTINEL ON)\n";
    }
    {
        std::ofstream stream(generatedCMakePath, std::ios::binary | std::ios::trunc);
        stream << "# stale generated cmake\n";
    }
...

    {
        ...
        buffer << stream.rdbuf();
        const std::string text = buffer.str();
        EXPECT_NE(text.find("# user root cmake"), std::string::npos);
        EXPECT_EQ(text.find("generated/AppProjectConfig.cpp"), std::string::npos);
    }
    {
        std::ifstream stream(userProjectCMakePath, std::ios::binary);
        ...
        buffer << stream.rdbuf();
        const std::string text = buffer.str();
        EXPECT_NE(text.find("USER_CMAKE_SENTINEL"), std::string::npos);
    }
    {
        std::ifstream stream(generatedCMakePath, std::ios::binary);
        std::stringstream buffer;
        buffer << stream.rdbuf();
        const std::string text = buffer.str();
        EXPECT_EQ(text.find("stale generated cmake"), std::string::npos);
        EXPECT_NE(text.find("generated/AppProjectConfig.cpp"), std::string::npos);
        EXPECT_NE(text.find("generated/MainView.cpp"), std::string::npos);
        EXPECT_NE(text.find("generated/TitleBarView.cpp"), std::string::npos);
    }
...
TEST(EditorSelectionTest, WorkspaceControllerReinitializeMainCppPreservesOldContentAndAppendsFreshTemplate)
{
    auto shellHost = std::make_shared<EditorShellHost>();
    auto documentTabs = std::make_shared<ImTabView>();
    auto projectView = std::make_shared<ImTextOutlineView>();
    ...
    const std::filesystem::path tempRoot =
        std::filesystem::temp_directory_path() / "imwidgetv4_editor_project_reinitialize_main_cpp";
    std::error_code errorCode;
    std::filesystem::remove_all(tempRoot, errorCode);
    std::filesystem::create_directories(tempRoot, errorCode);
    ASSERT_FALSE(errorCode);

    ASSERT_TRUE(workspaceController->CreateAppProjectAt(tempRoot, "ReinitApp"));
    const std::filesystem::path projectRoot = tempRoot / "ReinitApp";
    const std::filesystem::path mainCppPath = projectRoot / "src" / "main.cpp";

    {
        std::ofstream stream(mainCppPath, std::ios::binary | std::ios::trunc);
        stream
            << "#include <cstdio>\n\n"
            << "int customSentinel = 42;\n";
    }

    ASSERT_TRUE(workspaceController->ReinitializeMainCpp());

    {
        ...
        buffer << stream.rdbuf();
        const std::string text = buffer.str();
        EXPECT_NE(text.find("Previous main.cpp content was preserved"), std::string::npos);
        EXPECT_NE(text.find("// #include <cstdio>"), std::string::npos);
        EXPECT_NE(text.find("// int customSentinel = 42;"), std::string::npos);
        EXPECT_NE(text.find("FGeneratedAppHostDelegate"), std::string::npos);
        EXPECT_NE(text.find("GeneratedApp::BuildHostConfig()"), std::string::npos);
        EXPECT_NE(text.find("GeneratedApp::ConfigureApplication(application)"), std::string::npos);
        EXPECT_NE(text.find("// void ConfigureBackend"), std::string::npos);
